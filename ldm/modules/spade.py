...
class SPADE_cat(nn.Module):
    # norm_nc: output   label_nc: input
    def __init__(self, norm_nc, label_nc, config_text='spadeinstance3x3', norm=False):
        super().__init__()
    ...
    def forward(self, x_dic, segmap_dic, size=None):
        ...
        # Part 2. produce scaling and bias conditioned on semantic map
        # segmap = F.interpolate(segmap, size=x.size()[2:], mode='nearest')
        input = torch.cat([normalized, segmap],dim=1)
        actv = self.mlp_shared(input)
        gamma = self.mlp_gamma(actv)
        beta = self.mlp_beta(actv)
